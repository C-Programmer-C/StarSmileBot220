    ...
    async with lock:
        ...
        if not task_id:
            ...
            if pending_task_id:
                ...
                if task_id:
                    logger.info(
                        f"Found existing appeal task {task_id} for user {tg_id} on second check"
                    )
                    ...
                    if found_task_id:
                        if found_task_id != created_task_id:
                            ...
                            await notify_operators_anomaly(
                                int(found_task_id),
                                f"Конфликт обращений по tg_id={tg_id}: только что создана задача {created_task_id}, в реестре уже есть обращение {found_task_id}. Используется задача {found_task_id}.",
                                log_event="appeal_id_conflict",
                            )
        ...
        if task_id and fields_dict:
            await operator_warn_telegram_flow(tg_id, fields_dict)
...
async def input_telephone_handler(message: Message, state: FSMContext):
    ...
    async with lock:
        ...
        user_registry_conflict = False
        conflict_found_user_id: int | None = None
        ...
        for attempt in range(max_retries):
            ...
            if found_user_id:
                if found_user_id != created_user_id:
                    ...
                    user_registry_conflict = True
                    conflict_found_user_id = found_user_id
                    ...
                    if found_task_id:
                        if found_task_id != created_task_id:
                            ...
                            await notify_operators_anomaly(
                                int(found_task_id),
                                f"Конфликт обращений по tg_id={tg_id}: только что создана задача {created_task_id}, в реестре уже есть обращение {found_task_id}. Используется задача {found_task_id}.",
                                log_event="appeal_id_conflict_reg",
                            )
                        task_id = found_task_id
                        break
